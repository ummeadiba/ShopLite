...
function indexPage() {
  ...
  const body = [
    ...
    'below is the oracle candidates test against.</p>',
    '<div class="cards">',
    cards,
    '</div>',
    '<h2 id="run-it">Run the app</h2>',
    '<pre><code>git clone https://github.com/ummeadiba/ShopLite.git',
    'cd ShopLite',
    'node tools/static-server.js</code></pre>',
    '<p>Then open <code>http://127.0.0.1:5173/login.html</code> and sign in as',
    '<code>qa@shoplite.test</code> / <code>Passw0rd!23</code>.</p>',
    '<div class="notice">',
    '<strong>The defects are the point.</strong> Every bug in <code>app/</code> is',
    'planted on purpose &mdash; missing auth guards, XSS sinks, broken totals, logged',
    'passwords. Do not deploy this app anywhere public, and do not copy its patterns',
    'into anything real.',
    '</div>'
  ].join('\n');
